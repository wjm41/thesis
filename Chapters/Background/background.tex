...
The popularity of the Morgan fingerprint owes to its usefullness in calculating molecular similarity \cite{Maggiora2014similarity}and thus similarity-based virtual screening \& clustering, as well as its versatility as a molecular descriptor for machine learning. Intuitively, we would expect two molecules that have `similar' molecular fingerprints to have similar chemical structures (and potentically similar biological activity). Numerically, we can quantify the similarity between two molecular fingerprints by the Tanimoto coefficient \cite{Willet1998similarity}:
...
chemical database substructure searching (2) , similarity searching, (3) clustering, (4) and classification. (5), predicting and gaining insight into drug activity. (6) Additionally, ECFPs can be used much like other fingerprints in methods, such as similarity searching, clustering, and virtual screening. ECFP4 belongs to the best performing fingerprints in small molecule virtual screening [13]
