...
The most popular similarity measure for comparing chemical structures represented by means of fingerprints is the Tanimoto (or Jaccard) coefficient S (Eqn~\ref{eqn:tanimoto}). Two structures are usually considered similar if T > 0.85 (for Daylight fingerprints). However, it is a common misunderstanding that a similarity of T > 0.85 reflects similar bioactivities in general ("the 0.85 myth").[11]

Tanimoto similarity.

\begin{equation} \label{eqn:tanimoto} %TOOD - replace with intersection symbol
    S(A, B) = \frac{A \cup B}{A \cup B}
\end{equation}
