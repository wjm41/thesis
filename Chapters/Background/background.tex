...
Examples of RF with morgan fingerprint \cite{Svetnik2003RandomForest}. Bioactivity of kinase inhibitors \cite{Merget2017KinaseInhibitors}, predicting solubity \cite{Palmer2007RandomForest} and toxicity \cite{Polishchuk2009RandomForest}.
...
GPR in drug discovery \cite{Swamidass2005kernels}. GPR with tanimoto kernel \cite{Griffiths2022photoswitches}. GPR with expensive features for reaction barriers \cite{Jorner2021ActiviationEnergiesGPR}.

\subsection{Deep Learning}

In contrast to methods which uses hand-crafted features (`shallow learning'), deep learning revolves around learning representations directly from the raw data using neural networks. Neural networks are composed of layers of `neurons' that successively perform non-linear transformations on their inputs, mimicking the way that biological neurons transfer signals to one another. These transformations are typically of the form:
